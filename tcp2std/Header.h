...
{
public:
	CowsAndBulls_Player(int len) { LEN = len; fillPool(); }
	...
	void clearPool(string gs, pair<int, int>& r)
	{
		...
		while (pi != pool.end())
		{
			...
			if (!(GetTickCount() % 1000)) cout << "Progress: " << *pi << endl;
		}
	}
	...
	void fillPool()
	{
		for (int x = pow(10, LEN - 1); x < pow(10, LEN) - 1; x++)
		{
			ostringstream oss; oss << x;
			if (check(oss.str())) pool.push_back(oss.str());
			if (!(GetTickCount() % 1000)) cout << "Progress: " << x << endl;
		}
	}
...
// {
// 	srand(static_cast<unsigned int>(time(NULL))); CowsAndBulls_Player cb(6);
